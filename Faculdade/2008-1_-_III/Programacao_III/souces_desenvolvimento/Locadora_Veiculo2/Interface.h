...
private:
	...
	static void escreverDadosLocacao(Locacao *locacao, Persistencia *locadora);
	static bool veiculoLocado(Locacao *locacao, Persistencia *locadora);
	static void atribuirStuacaoVeiculo(Locacao *locacao, Persistencia *locadora);
...
void Interface::lerDadosVeiculo(Veiculo *veiculo)
{
	...
	// TODO CADASTRO O VEICULO NAO ESTARA LOCADO POR DEFAULT
			...
			switch(opcao){
				case 1:{
					try{
						...
						//se o veiculo estiver locado
						if(veiculoLocado(&locacao, locadora)){
							//cout<<" Veiculo jah esta locador!"<<endl;
							cout<<"Locacao cancelada - Veiculo Locado!"<<endl;
							cin.get();cin.get();
							break;
						}
						
						// alterando o status do veiculo para locado
						atribuirStuacaoVeiculo(&locacao, locadora);
						
						//alterando o status da locacao como nao devolvido
						locacao.atribuirDevolvido(false);
				...
				case 2:{ // Devolucao
					try{
						...
						//nao existe devolucao pra fazer com este id
						if(locacao.obterDevolvido()) {
							cout<<" Locacao devolvida - DEVOLUCAO CANCELADA!"<<endl;
							cout<<" Tecle enter para continuar!"<<endl;
							cin.get();cin.get();
							break;
						}
						
						//ativando esse atributo para nao devolver novamente o mesmo veiculo
						
						
						escreverDadosLocacao(&locacao, locadora);
						...
						//atualizando os dados da locacao
						//alterando o status da locacao
						locacao.atribuirDevolvido(true);
						if(!ctrlLocacao.alterarLocacao(&locacao)){
							throw string("Ocorreu um Erro ao Atualizar os dados da locacao!");
						}
				...
				case 3:{ // cancelar
					try{
						...
						//nao existe devolucao pra fazer com este id
						if(locacao.obterDevolvido()) {
							cout<<" Locacao devolvida - CANCELAMENTO NAO EFETUADO!"<<endl;
							cout<<" Tecle enter para continuar!"<<endl;
							cin.get();cin.get();
							break;
						}
						...
						cout<<" Confirma a Cancelamento[s/n]?: ";
						cin.ignore();
						char resp;
						cin>>resp;
						if(resp!='s' && resp!='S') resp = 'n';
						...
						if(resp=='s' || resp=='S'){
							...
							if(ctrlVeiculo.consultarVeiculo(&veiculo)){
								veiculo.atribuirSituacao(false);
								if(ctrlVeiculo.alterarVeiculo(&veiculo)){
									cout<<" Veiculo Cancelado com Sucesso"<<endl;
									cout<<" Enter Para Continuar!"<<endl;
									//cin.get();cin.get();
								}///fim if
							}//fim consulta
							...
							cout<<endl<<" CANCELAMENTO CANCELADO =P!"<<endl;
							cout<<" Tecle enter para continuar!"<<endl;
							cin.get();cin.get();
							break;
						}
...
//metodo para  o uso da validacao na hora da locacao
bool Interface::veiculoLocado(Locacao *locacao, Persistencia *locadora)
{
	// feito para verificar se o veiculo esta locado
	ControleVeiculo ctrlVeiculo(locadora);
	int idVei = locacao->obterIdVeiculo();
	Veiculo vei(idVei);
	if(ctrlVeiculo.consultarVeiculo(&vei)){
		if(vei.obterSituacao()) { // se a situacao estiver tru eh pq tah locado
			return true;
		}//fim if
	}
	return false;
...
//metodo responsavel para alterar o id do status do veiculo

void Interface::atribuirStuacaoVeiculo(Locacao *locacao, Persistencia *locadora)
{
	ControleVeiculo ctrlVeiculo(locadora);
	Veiculo vei(locacao->obterIdVeiculo());
	if(!ctrlVeiculo.consultarVeiculo(&vei)){
		throw string("Atribuir Situacao - Veiculo Nao encontrado");
	}
	
	vei.atribuirSituacao(true);
	if(!ctrlVeiculo.alterarVeiculo(&vei)){
		throw string("Erro na alteracao!");	
	}
...
void Interface::lerDadosLocacao(Locacao *locacao, Persistencia *locadora)
{
	...
	do{
		...
		locado = veiTmp.obterSituacao();
		
		if((!encontrado && (auxInt!=-1) )) cout<<" Veiculo nao encontrado. Digite um ID valido!"<<endl;
		if(locado) cout<<" Veiculo Locado: informe outro ou -1 pra sair!"<<endl;
		
		
		if(auxInt==-1){
			throw string("Processo de Inclusao Cancelado!");
			encontrado = true;
		}
		
		// repetir enquanto:1 digitar veiculo valido e um veiculo locado	
	}while((!encontrado) || (locado));
...
void Interface::escreverDadosLocacao(Locacao *locacao, Persistencia *locadora)
{
	ControleCliente ctrlCliente(locadora);
	ControleVeiculo ctrlVeiculo(locadora);
