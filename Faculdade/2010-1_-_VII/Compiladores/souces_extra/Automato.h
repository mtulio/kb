...
std::string Automato::iniciarAutomato()
{
	...
	while(!persistencia.fimDoArquivo()){
		...
		switch(qtdElementos){
			...
			case 0:{//nao existe nenhum elemento para ser lido
				//return ("-1");
				break;
			}//fim case 0
			...
			case 1:{
				...
				for(i=0,j=0; linha[i]!='\0'; i++,j++){
					...
					if(linha[i]!= ' ' && linhaQuebrada[0] != '\0'){
						...
						strTemp2 = chtemp;
						if(msgRetorno!=("-1")) return ("linha:"+strTemp2+":iniA-c1. "+strTemp+". "+msgRetorno);
					}//fim if
					i=j;
				}//fim for
					...
					case 2:{
						...
						for(i=0,j=0; linha[i]!='\0'; i++,j++){
							...
							if(i>posPrimeiroParenteses && i<posSegundoParenteses){
								...
								int inicioTexto = posPrimeiroParenteses+2,
