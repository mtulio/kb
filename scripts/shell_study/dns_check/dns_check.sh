...
##>
echo "###> Starting test at $(date)" |tee -a $FILE_LOG
...
function test_server_QTIME() {
  echo -ne "# Running server test - Query Time\t: "
...
function test_server_VERSION() {
  ...
  echo -ne "# Running server test - Bind version\t: "
  ...
  QUERY_ANSWER=$(cat ${FILE_TMP} |grep -v -e ^$ -e ^';' |awk -F'TXT	' '{print$2}')
  #echo "QUERY_STATUS=[$QUERY_STATUS]"
  #echo "QUERY_ANSWER=[$QUERY_ANSWER]"
  if [ "${QUERY_STATUS}" == "NOERROR" ]; then   echo "WARN [Version is $QUERY_ANSWER . Disable or mask this information]";
  elif [ "${QUERY_STATUS}" == "SRVFAIL" ]; then echo "[NOT AVAILABLE]";
...
function test_server_RECURSION() {
  ...
  echo -ne "# Running server test - Recursion\t: "
  RET_MSG="$(dig @${NAMESERVER} google.com A +noquestion +noauthority +noadditional +nostats > dig.txt)"
  QUERY_STATUS=$(cat dig.txt |grep "HEADER" |awk -F', ' '{print$2}' |awk '{print$2}')

  
  if [ "${QUERY_STATUS}" == "NOERROR" ]; then   echo "WARNING [RECURSION enabled] "
  elif [ "${QUERY_STATUS}" == "REFUSED" ]; then echo "OK [NOT AVAILABLE]"
  else echo "[NO INFORMATION]";
  fi
}
...
function test_server_USETCP() {
  ...
  echo -ne "# Running server test - Is using TCP\t: "
  RET_MSG="$(dig @${NAMESERVER} version.bind chaos txt +noquestion +noauthority +noadditional +nostats +tcp > ${FILE_TMP}; export RET_CODE=$?)"
  QUERY_STATUS=$(cat ${FILE_TMP} |grep "HEADER" |awk -F', ' '{print$2}' |awk '{print$2}')
  #echo "QUERY_STATUS=[$QUERY_STATUS]"

  if [ "${QUERY_STATUS}" == "NOERROR" ]; then   echo "OK [TCP queries is enabled]";
  elif [ "${QUERY_STATUS}" == "SERVFAIL" ]; then echo "FAIL [NOT AVAILABLE - Queries higher than 512B use TCP port 53]";
  else echo "WARN [NO INFORMATION of TCP]";
  fi
...
function test_domain_AXFR() {
  ...
  echo -ne "# Running domain test - Zone transfer\t: "
  RET_MSG="$(dig @${NAMESERVER} ${DOMAIN} AXFR +noquestion +noauthority +noadditional +nostats +tries=1 > ${FILE_TMP}; export RET_CODE=$?)"

  if [ "$(grep 'Transfer failed' ${FILE_TMP} >/dev/null 2>&1; echo $?)" == "0" ]	; then   echo "OK [DENIED by configuration]";
  elif [ "$(grep 'connection timed out' ${FILE_TMP} >/dev/null 2>&1; echo $?)" == "0" ]	; then   echo "OK [DENIED by network/firewall]";
  elif [ "$(grep -v -e ^$ -e ^';' ${FILE_TMP} >/dev/null 2>&1)"x != "x" ]	; then   echo "FAIL [WARNING Domain transfer is enabled, you must fix it]";
  else echo "[NO INFORMATION]";
  fi
...
function test_domain_SOA() {
  ...
  echo -ne "# Running domain test - records SOA\t: "
  ...
  if [ "${QUERY_STATUS}" == "REFUSED" ]; then   
	echo "WARN [Server isn't authoritative for this domain]";
  elif [ "${QUERY_STATUS}" == "NOERROR" ]; then
	echo "OK";
	echo -e "# Running domain test - records SOA\t: '-> P.SERVER \t[$(echo $QUERY_ANSWER |awk '{print$1}')]"
	echo -e "# Running domain test - records SOA\t: '-> EMAIL \t[$(echo $QUERY_ANSWER |awk '{print$2}')]"
	echo -e "# Running domain test - records SOA\t: '-> SERIAL \t[$(echo $QUERY_ANSWER |awk '{print$3}')]"
	echo -e "# Running domain test - records SOA\t: '-> REFRESH \t[$(echo $QUERY_ANSWER |awk '{print$4}')]"
	echo -e "# Running domain test - records SOA\t: '-> FAIL REF \t[$(echo $QUERY_ANSWER |awk '{print$5}')]"
	echo -e "# Running domain test - records SOA\t: '-> EXPIRE \t[$(echo $QUERY_ANSWER |awk '{print$6}')]"
	echo -e "# Running domain test - records SOA\t: '-> NEG TTL \t[$(echo $QUERY_ANSWER |awk '{print$7}')]"
  else 
	echo "[NO INFORMATION]";
  fi
}
...
function test_domain_NS() {
  ...
  echo -ne "# Running domain test - records NS\t: "
  ...
  if [ "${QUERY_STATUS}" == "REFUSED" ]; then   
	...
	do
		echo -e "# Running domain test - records NS\t: '-> $VALUE [$(host $VALUE |awk -F'address ' '{print$2}'|tr '\n' ' ')]"
	done
...
function test_domain_MX() {
  ...
  echo -ne "# Running domain test - records MX\t: "
  ...
  if [ "${QUERY_STATUS}" == "REFUSED" ]; then   
	...
	do
		echo -e "# Running domain test - records MX\t: '-> $VALUE [$(host $VALUE |awk -F'address ' '{print$2}'|tr '\n' ' ')]"
	done
  else 
	echo "[NO INFORMATION]";
  fi
...
function test_domain_A() {
  ...
  echo -ne "# Running domain test - records A\t: "
...
function test_domain_DSEC_KEY() {
  ...
  echo -ne "# Running domain test - NSec KEY KSK\t: "

  if [ "$(dig +multiline ${DOMAIN} DNSKEY +short)x" == "x" ]; then
    echo "FAIL [DNSsec is not available for this domain]"
    return;
  fi
  ...
  if [ "${QUERY_STATUS}" == "REFUSED" ]; then   
	echo "FAIL [DNS Sec is not configured for this domain]";
  elif [ "${QUERY_STATUS}" == "NOERROR" ]; then
	echo "OK [KSK key id is: $(dig @${NAMESERVER} ${DOMAIN} DNSKEY +multiline +tries=1 |grep KSK |awk -F'key id = ' '{print$2}')] [Server: $(grep 'SERVER:' ${FILE_TMP} |awk -F';; SERVER: ' '{print$2}')]"
  else 
	echo "[NO INFORMATION]";
  fi
}
...
function test_domain_DSEC_KEY_PAR() {
  ...
  echo -ne "# Running domain test - NSec PARENT KSK\t: "
  ...
  if [ "${QUERY_STATUS}" == "REFUSED" ]; then   
	...
	echo "OK [KSK key id is: $(dig ${DOMAIN} DS +multiline  +short |awk '{print$1}' |head -n1)] [Server: $(grep 'SERVER:' ${FILE_TMP} |awk -F';; SERVER: ' '{print$2}')]"
  else 
	echo "[NO INFORMATION]";
  fi
}
